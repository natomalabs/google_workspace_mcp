...
class MCPSessionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Any:
        ...
        try:
            ...
            # Continue without session context - let FastMCP handle the request
            # Don't raise the exception as it might cause FastMCP to return invalid responses
            try:
                return await call_next(request)
            except Exception as inner_e:
                # If call_next also fails, log it but don't interfere with FastMCP's error handling
                logger.error(f"Error in call_next after middleware error: {inner_e}", exc_info=True)
